// HeaderHashCLI is a Golang command-line tool that computes a SHA-256 hash for HTTP/HTTPS response headers.
// It sends a GET request to a specified URL, concatenates the header keys, and generates the hash.
// Ensure the integrity of HTTP headers with this lightweight and efficient utility.
...
// MakeHTTPRequest is a function that takes a raw URL string as input, parses the URL,
// establishes a TCP connection based on the scheme (HTTP or HTTPS),
...
// GenerateHHHash is a function that takes a slice of header keys,
// concatenates them with a colon (:) between elements, and computes a SHA256 hash of the resulting string.
// The hash is returned as a string.
func GenerateHHHash(headers []string) string {
	data := strings.Join(headers, ":")
	hash := sha256.Sum256([]byte(data))
	return fmt.Sprintf("HashValue:1:%x", hash)
}
